/**
 * Scroll-progress bridge: exposes the page's scroll position as plain mutable
 * refs, so any consumer can read it every frame WITHOUT a second scroll listener
 * and WITHOUT triggering React re-renders.
 *
 * The ScrollTriggers are created inside the existing gsap.context (see
 * useMotion), so they share the one Lenis-synced ScrollTrigger instance
