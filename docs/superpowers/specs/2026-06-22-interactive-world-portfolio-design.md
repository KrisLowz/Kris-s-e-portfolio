...
- **Physics (interaction phase only):** reuse **`matter-js`** — already installed (`^0.20.0`, with `@types/matter-js`) from the existing motion work. Run it in **screen-space 2.5D** (shards simulate in a camera-facing plane; they still rotate in 3D visually). No new dependency. Physics is needed only for the *mouse-play* phase; the scroll-driven smash itself is a deterministic, reversible procedural animation (no engine).
- **Interactivity reality:** the background `<Canvas>` is `pointer-events:none` and sits *behind* the DOM; today the established pattern is **DOM-drives-3D** via the `worldEvents` bus. Directly grabbing 3D objects therefore requires a dedicated DOM hit-layer over the `#skills` section that feeds matter-js — deferred to the interaction phase.
...
**Phase 1 — the smash cinematic (no engine, fully reversible):** shard transforms are a **pure function of `sections.skills` scroll progress** + a time term for drift. Planet cracks grow → at a detonation threshold the planet dissolves and 17 iridescent shards fly out along seeded directions → gentle zero-g drift → past ~0.85 they streak/fade toward the Experience beacon. Scrolling back recomputes the same states, so it re-assembles for free. Runs only where the scene runs (capable desktop). This is the foundation and is built first.

**Phase 2 — the hands-on play:** a DOM hit-layer over `#skills` (pointer-events auto) feeds a **matter-js** screen-space sim during the dwell; the 3D shards mirror the body transforms. Hover → bloom + label; drag → throw; click → focus + orbital holo-fragments (**name · category · one-line blurb · project chip e.g. 🏆 TrackPoint · signal-strength bar from `level`**). This is where the `Skill` data enrichment lands and where the old `HoloSkills` DOM is replaced.

**Content:** the 17 skills — HTML5, CSS3, JavaScript, Python, Java, C++, C#, SQL (inner) · Figma, Tailwind CSS, PostgreSQL, Firebase, Kotlin, Flutter, Android, Git, VS Code (outer). Example links: Kotlin/Firebase/Android → TrackPoint; Python/JavaScript → Cinemate & Splash Aquatics; SQL/PostgreSQL → Splash Aquatics.

**Fallback (mobile / `prefers-reduced-motion`):** the scene never mounts there (`shouldRenderScene` gate), so the existing **`HoloSkills` DOM `#skills` section remains the universal readable fallback** — first-class, not an afterthought. No separate faked-scatter layer is needed for v1.
