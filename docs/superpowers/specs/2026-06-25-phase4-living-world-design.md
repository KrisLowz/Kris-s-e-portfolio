# Phase 4 — Living World (Mascot, Finale, Chatbot) — Design Spec

**Date:** 2026-06-25
**Status:** Approved in brainstorming; ready for implementation planning (writing-plans, starting with 4a).
**Parent:** `docs/superpowers/specs/2026-06-24-cosmic-cyberpunk-voyage-redesign-design.md` (§9 Phase 4; §10 chatbot kept/reskinned).
...
**Builds on:** Phases 0–3 on `v2` (readable voyage → cinematic motion layer → interactive Forge + project portals). The spaceship travel between sections already shipped in Phase 2a; the chatbot was deleted at teardown and is rebuilt here.
...
3. **The comms channel returns.** The AI chatbot — removed at teardown — is rebuilt as a floating cat-comms assistant.
...
- **Internal phasing: 4a (living-world finale) then 4b (cat-comms chatbot).** Each gets its own writing-plans plan; 4a is built first.
- **Chatbot persona:** professional / recruiter-accurate with a thin cat-comms voice (not a gimmick) — the bot must still answer hiring questions usefully.

**Not in Phase 4:** multi-pose mascot art (future), the Phase-5 cleanup (dead CSS / unused deps / accumulated Minor notes). No new portfolio data (the `STORY_ACTS`/`MascotState`/chatbot system prompt already exist).
...
## 3. Phase 4b — Reskinned cat-comms chatbot
...
- No ScrollTrigger `pin` anywhere; mascot travel is a scrubbed (non-pinned) timeline like the ship; the signal beam + ending are click/trigger-driven one-shots; the chatbot is plain React.
...
- Chatbot is usable with keyboard + screen reader and degrades without an API key.

---

## 5. Internal phasing + testing

- **Phase 4a — living-world finale:** `MascotPilot` (+ `mascot` toggle + active-act detection), `SignalBeam` on contact submit, and the ending departure choreography. First shippable slice.
- **Phase 4b — cat-comms chatbot:** `geminiService` + the floating widget + the (lightly recast) system prompt + graceful fallback.

Each gets its own writing-plans plan; the first plan covers **4a**.

**Verification:** `tsc`/`vitest`/build clean; live desktop walkthrough (mascot rides the ship and its mood/PILOT line changes per act; contact submit → beam + celebrate + success; ending → ship departs + mascot waves; **no `insertBefore` crash** = no accidental pin; `ScrollTrigger.getAll().filter(t=>t.pin).length === 0`); live mobile + reduced-motion (static cat, plain success, plain footer); the chatbot answers a portfolio question when a key is present and shows the offline fallback when it is not; keyboard operates the chat.

---

## 6. Scope boundaries (YAGNI)

- **In, Phase 4:** the mood-cat mascot (4a), the signal beam + ending finale (4a), and the rebuilt cat-comms chatbot (4b), plus their fallbacks.
- **Out:** multi-pose mascot art; sound; the Phase-5 cleanup (dead CSS, unused deps, accumulated Minor notes); any new portfolio data.
...
4. How far to recast the chatbot persona toward "cat" without harming recruiter usefulness (the proposed default stays professional with a thin cat voice).
